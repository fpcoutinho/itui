...
- **O `PATCH` do perfil é PATCH mesmo: só viaja o que o formulário editou.** O backend distingue campo ausente (não mexe) de `null` (limpa), e `updateProfile` preserva os dois: chave com `undefined` sai do corpo, `null` e string em branco viram `null`. Quem chama manda **apenas o campo da linha em edição** — repetir os outros para "não perdê-los" transformaria a rota em `PUT` e é exatamente o que apagava o `avatar_url` vindo do Google a cada gravação de nome. Um input esvaziado tem que virar `null` explícito, nunca `undefined`, senão a limpeza vira silêncio e o valor antigo volta no próximo `GET`.
...
- **Botão nunca se escreve à mão.** Ação é `UaButton`; navegação interna é `ButtonLink` (`src/components/ui/ButtonLink.tsx`), que veste o `<Link>` do react-router com as classes do `UaButton`. O `behavior="link"` do pacote existe, mas renderiza `<a href>` puro — reload de página inteira, e o access token vive só em memória, então cada clique derrubaria a sessão. Por isso **`ButtonLink` é o único arquivo do repo acoplado aos nomes de classe do Sanhauá** (`ua-button`, `.text`, `.icon`); nenhuma tela repete essa marcação. Se o pacote ganhar uma prop polimórfica (`as`), a troca acontece só ali.
  ...
  **Nenhuma regra daqui pode selecionar classe do Sanhauá** (`.ua-*` e as classes internas que o pacote usa). Sem exceção: `grep -rE "\.ua-[a-z-]+" src --include=*.scss` só pode casar em comentário.

  Quando o componente do DS precisa de ajuste no contexto, o caminho é **passar uma classe nossa na instância** (todos os componentes aceitam `className` no elemento raiz) e estilizar essa classe — `.on-accent` no `ButtonLink` do `.cta`, `.choice-option` no `UaCheckbox` do `SchemaField`. O nome tem que ser nosso e específico: `.option` casaria com o miolo do `UaSelect`. Como as regras do pacote são de 4 classes (`.sanhaua.light .ua-button.ghost`), o seletor precisa chegar a 4 para vencer — daí o `.cta-actions` no contêiner. Antes disso, considerar se não é caso de mudar o contexto: o alternador de tema da conta saiu de cima do gradiente em vez de ser repintado (ver `.account .theme`).
