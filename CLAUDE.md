...
- `src/components/ui/` — componentes base acoplados ao contexto da aplicação (`PageHeader`, `Sidebar`, `ThemeToggle`, `ButtonLink`, `ConfirmDialog`). `Card` e `Banner` moravam aqui e saíram na `0.15.0`, virando `UaCard` e `UaAlert` no pacote.
